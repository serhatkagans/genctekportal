...
export function ThemeOfficialProgram({ program, arsivVar = false }: { program: ThemeProgram; arsivVar?: boolean }) {
  return <section className="official-program"><div className="container">
    ...
    <div className="official-program-body">
      ...
      <section className="official-program-highlights"><span className="theme-list-label">Programda öne çıkanlar</span><ul>{program.highlights.map((item)=><li key={item}>{item}</li>)}</ul>{arsivVar ? <Link className="button button-secondary" href={`#${TEMA_ARSIV_CAPASI}`}>{program.sourceLabel} ↓</Link> : <Link className="button button-secondary" href={program.sourceUrl} target="_blank" rel="noreferrer">{program.sourceLabel} ↗</Link>}</section>
    </div>
  </div></section>;
}
